...
> [We run](www.jumpingrivers.com) courses on R, Stan, Scala, Shiny and 
> Python. We also provide in-house training. Get a 10% discount for courses booked
> before Jan 1st 2017. Just mentioned "useR Efficient" in the booking notes^[The discount
is valid with the standard academic rate; but can't be used with other discounts.].

## Exercise 1

In this example, we are going to investigate loading a
...
1.  Use the `saveRDS()` and `readRDS()` functions:
    ...
    ```
1. Install the __readr__ package via
    ```{r eval=FALSE}
...
Then load the package in the usual way
    ...
    library("readr")
    ```
This package contains the function `read_csv()`; a replacement function for `read.csv()`. 
